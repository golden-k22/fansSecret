...
import { handleFollowing } from '../home/home-screen/actions';
...
function SubscriptionModal({ admin, post, setIsSubscribed, isFollowing, setIsFollowing }: SubscriptionProps) {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const { toast } = useToast();

    const handleButtonClick = () => {
        if (post.isPublic && !isFollowing) {
            if (admin && admin.id) {
                handleFollowing(post.userId, admin.id).then(() => setIsFollowing(true));
            }
        } else {
            setIsModalOpen(true);
        }
