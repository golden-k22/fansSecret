...
import { CheckIfFollowing } from "../home/home-screen/actions";
...
const Post = ({ post, admin }: { post: PostWithComments; admin: User }) => {
	...
	useEffect(() => {
		async function checkIsFollowing() {
			if (poster) {
				const val = await CheckIfFollowing(poster.id, admin.id);
				setIsFollowing(val);
			}
		}
		checkIsFollowing();
	...
	return (
		<div className='flex flex-col gap-3 p-3 border-t'>
			...
			{(post.isPublic && isFollowing || isSubscribed) && post.mediaUrl && post.mediaType === "image" && (
				<div className='relative w-full pb-[56.25%] rounded-lg overflow-hidden'>
					<Image src={post.mediaUrl} alt='Post Image' className='rounded-lg object-cover' fill />
				</div>
			)}

			{(post.isPublic && isFollowing || isSubscribed) && post.mediaUrl && post.mediaType === "video" && (
				<div className='w-full mx-auto'>
					<CldVideoPlayer width='960' height={540} className='rounded-md' src={post.mediaUrl} />
				</div>
			)}

			{((!isSubscribed && !post.isPublic) || (post.isPublic && !isFollowing)) && (
				<div
        ...
        '
				>
					...
					<div className='flex flex-col gap-2 z-10 border p-2 border-gray-500 w-full rounded'>
						...
						<SubscriptionModal admin={admin} post={post} setIsSubscribed={setIsSubscribed} isFollowing={isFollowing} setIsFollowing={setIsFollowing} />
